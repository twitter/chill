...
lazy val chillHadoop = module("hadoop")
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      "org.slf4j" % "slf4j-log4j12" % "1.7.33" % "provided"
    )
  )
  .dependsOn(chillJava)
...
lazy val chillThrift = module("thrift").settings(
  ...
  autoScalaLibrary := false,
  libraryDependencies ++= Seq(
    "org.apache.thrift" % "libthrift" % "0.15.0" % "provided"
  )
)
...
lazy val chillScrooge = module("scrooge")
  .settings(
    crossScalaVersions += "2.13.7",
    libraryDependencies ++= Seq(
      ("org.apache.thrift" % "libthrift" % "0.15.0").exclude("junit", "junit"),
